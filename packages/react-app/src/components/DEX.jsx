...
export default function DEX(props) {
  ...
  const tokenBalance = useTokenBalance(props.readContracts[tokenName], contractAddress, 1777);
  console.log({ contractAddress, tokenBalance: tokenBalance.toString() });
  ...
  if (props.readContracts && props.readContracts[contractName]) {
    display.push(
      <div key="rowContainer" id="rowContainer">
        {rowForm("ethToToken", "💸", async value => {
          let valueInEther = ethers.utils.parseEther("" + value);
          let swapEthToTokenResult = await props.writeContracts[contractName]["ethToToken"]({
            from: props.address,
            value: valueInEther,
        ...
        {rowForm("tokenToEth", "🔏", async value => {
          ...
          if (allowance.lt(valueInEther)) {
            approveTx = props.writeContracts[tokenName].approve(
              props.readContracts[contractName].address,
              valueInEther,
              { from: props.address, gasLimit: 200000, nonce: nonce++ },
            );
            console.log("approve tx is in, not waiting on it though...", approveTx);
          }
          let swapTx = props.writeContracts[contractName]["tokenToEth"](valueInEther, {
            from: props.address,
            gasLimit: 200000,
            nonce: nonce++,
          });
        ...
        {rowForm("deposit", "📥", async value => {
          ...
          let depositTx = props.writeContracts[contractName]["deposit"]({
            from: props.address,
            value: valueInEther,
            gasLimit: 200000,
            nonce: nonce++,
  ...
  console.log({
    ethBalanceFloat,
    tokenBalanceFloat,
    addingEth: values["ethToToken"],
    addingToken: values["tokenToEth"],
  });

  return (
    <Row>
      <Col>
        <div style={{ paddingTop: 20, paddingLeft: 80, paddingRight: 80 }}>
          <Curve
